...
int distance[Number_Of_Speakers];
int distance1[Number_Of_Speakers];
int Rank[Number_Of_Speakers];
...
void calculation()
{
    for(int i = 0 ; i<Number_Of_Speakers ; i++)
    {
        ...
        for ( j=0; j<Number_Of_Coefficients; j++ )
        {
                distance[i] = distance[i] + ((mfcc_vector[j]-training_vector[i][j])*(mfcc_vector[j]-training_vector[i][j]));
...
/* Function to calculate rank based on euclidean distance*/
void RankCalculation()
{
    int i,j,k; /* variables declared to act as counters */
    /* i = 40, j,k = 20 */
    for(int i = 0 ; i< Number_Of_Speakers ; i++)
    {
        for(int j = 0 ; j < Number_Of_Coefficients; j++)
        {
            for int k = 0 ; k < Number_Of_Coefficients; k++)
        }
            if(distance1[j] == distance[k] )
            {
                Rank[j] = k+1;
                break;
            }
